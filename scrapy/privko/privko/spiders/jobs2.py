...
# @About  : A script which scrapes site data from privco.com                                #
#                                                                                           #
#         : '../settings.py' file to configure various spider settings.                     #
#         : '../items.py' contains definitions for the e27company StackItem                 #
    ...
    def parse(self, response):
        for num in range(25):
            ...
            yield{
                    ...
                    'Investor_name_link' : quote.css('td.col-name span.profile-name a::attr(href)').extract_first(),
                    if not Investor_name_link:
                        item['Investor_name_link'] = None
                    else:
                        item['Investor_name_link'] = Investor_name_link
                    'country' : quote.css('td.col-country a::text').extract_first(),
                    if not country:
                        item['country'] = None
                    else:
                    ...
                    if not state_province:
                        item['state_province'] = None
                    else:
                        item['state_province'] = state_province
                    'Investor_type' : quote.css('td.col-investor-type a::text').extract_first()
                    if not Investor_name:
